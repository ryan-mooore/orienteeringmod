...
public class ControlBoxBlock extends BlockWithEntity {
    ...
    public static <T extends AbstractSportIdentItem> boolean isHoldingSportIdentOfType(Entity entity,
            Class<T> sportIdentItemType) {
        if (entity instanceof PlayerEntity) {
            ItemStack itemStack = ((PlayerEntity) entity).getStackInHand(((PlayerEntity) entity).getActiveHand());
            if (sportIdentItemType.isAssignableFrom(itemStack.getItem().getClass())) {
                return true;
            }
        }
        return false;
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand,
        ...
        if (isHoldingSportIdentOfType(player, AbstractSportIdentItem.class)) {
            ItemStack itemStack = player.getStackInHand(player.getActiveHand());
            AbstractSportIdentItem.punch(itemStack, (ControlBoxBlockEntity) blockEntity);
            return ActionResult.SUCCESS;
        }

        ((OpenControlBoxBlockScreenAccessor) player).openControlBoxBlockScreen((ControlBoxBlockEntity) blockEntity);
